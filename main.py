            ...
            if add_pill(event.obj.user_id, 1):
                try:
                    bot.messages.send(user_id=event.obj.user_id,
                                    ...
                                    random_id=randint(1, 99999999),
                                    keyboard=create_keyboard(event.obj.user_id))
                except:
                    pass
...
Отписываться нельзя! Тогда таблетка пропадет""",
                    ...
                    if not list_friend:
                        ...
                        for i in range(1, 6):
                            try:
                                ...
                                break
                        with open(str(event.obj.from_id) + ".txt", "w") as f:
                    ...
                    try:
                        ...
                        pass
                    with open(str(event.obj.from_id) + "_friend.txt", "w") as f:
                        f.write("False")
                    ...
                    try:
                        with open(str(event.obj.from_id) + "_friend.txt") as f:
                            is_top_friend = f.read().strip()
                        if is_top_friend == "True":
...
Ты зашел в опасную зону и заразился коронавирусом, чтобы излечиться, тебе нужно собирать таблетки, чтобы их получать заражай друзей"""
                            ...
                            try:
                                with open(str(event.obj.from_id) + ".txt"):
                                ...
                                if result[0]:
                                    add_life_kits(info[event.obj.text])

                                    try:
                                        bot.messages.send(user_id=event.obj.from_id,
                                        message=f"Вы успешно добавил 8 часов жизни своему @id{info[event.obj.text]}(другу)\n\n🧰 Осталось аптечек: {result[1]} ",
                                        random_id=randint(1, 99999999),
                                        keyboard=create_keyboard(event.obj.from_id))
                                    except:
                                        pass
                                    os.remove(str(event.obj.from_id) + ".txt")
                                else:
                                    try:
                                        bot.messages.send(user_id=event.obj.from_id,
...
for event in longpoll.listen():
    proc = Process(target=update_event, args=(event,))
    proc.start()
    proc.join()